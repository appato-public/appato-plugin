...
/**
 * The wire-contract version — the ONE hand-maintained CLI version. It moves
 * only when the CLI's contract with the platform does (the bump history and
 * the MIN floor live in src/version.ts). The platform advertises this same
 * number as "latest" via the generated src/cli-build.ts, so the served CLI
 * and its advertised version can never disagree. WHICH build you are running
 * is `BUILD` in cli/src/build_generated.mjs, derived from these sources and
 * so moving on every change.
 */
const VERSION = "0.12.0";

/**
 * Set by a plugin's bin/ launcher (plugin/bin/appato → "claude-code"). When
 * present, this copy of the CLI updates WITH the plugin, and `appato upgrade`
 * (which writes ~/.appato/bin) cannot replace it — every upgrade hint has to
 * say so, or the driving agent loops on an upgrade that changes nothing.
 */
const PLUGIN = process.env.APPATO_PLUGIN || "";
...
// Never uploaded: appato.json is metadata, and _appato.d.ts is local tooling
// generated by `appato types`; the real _appato.js is injected at deploy.
...
// ---------------------------------------------------------------------------
// cli/src/sdktypes_generated.mjs

// GENERATED FILE — do not edit. Authored in src/sdk/server.ts; rebuild with
// `npm run build:sdk-types`. `npm run verify` fails when this file is stale.

const SDK_TYPES = "// Appato server SDK types — generated by appato CLI v0.12.0 from the deployed SDK.\n// Do not edit. Refresh with `appato types`; verify with `appato types --check`.\n// This file is local tooling only: the Appato CLI never uploads or deploys it.\n\nexport type JsonValue = string | number | boolean | null | JsonValue[] | {\n    [key: string]: JsonValue;\n};\nexport interface AppatoWriter {\n    id: string;\n    name: string;\n}\nexport interface AppatoUser {\n    id: string;\n    email: string;\n    name: string;\n    org: string;\n    /** true for workspace members. Phase 1: always true. Phase 2: false for\n     * accounts that signed in through the app's public login (docs/PUBLIC.md). */\n    member: boolean;\n}\n/** The Worker execution context Appato passes to webhook and email handlers.\n * Kept structural so generated SDK types need no Cloudflare type package. */\nexport interface AppatoExecutionContext {\n    waitUntil(promise: Promise<unknown>): void;\n    passThroughOnException(): void;\n    readonly exports: unknown;\n    readonly props: unknown;\n    readonly cache?: unknown;\n    readonly access?: unknown;\n    readonly tracing: unknown;\n}\nexport interface CronContext {\n    name: string;\n    scheduledAt: number;\n    trigger: \"manual\" | \"schedule\";\n}\nexport type WebhookHandler<Env = unknown> = (request: Request, env: Env, ctx: AppatoExecutionContext) => Response | Promise<Response>;\n/** One entry as `entry(key)` returns it: the value plus who wrote it, when,\n * and its revision. No `key` — the caller passed it (list entries carry one\n * because a page holds many). */\nexport interface StorageValueEntry {\n    value: JsonValue;\n    by: AppatoWriter | null;\n    at: number;\n    /** 1 when the key was created, +1 on every write since. Pass it back as\n     * `ifRev` to write only if nobody else has written meanwhile. */\n    rev: number;\n}\nexport interface StorageEntry extends StorageValueEntry {\n    key: string;\n}\nexport interface StorageListOptions {\n    limit?: number;\n    reverse?: boolean;\n    after?: string;\n}\n/** The conditional-write precondition (compare-and-swap). Omit `ifRev` for\n * today's unconditional write; pass a revision to require the entry still be\n * at exactly that one; pass `null` to require the key be ABSENT (an atomic\n * create/claim). A mismatch writes nothing and throws AppatoRevConflictError. */\nexport interface StorageWriteOptions {\n    ifRev?: number | null;\n}\n/** Thrown by a conditional `set`/`delete` whose `ifRev` did not match. `rev`\n * is the entry's CURRENT revision, or null when the key does not exist. */\nexport interface AppatoRevConflictError extends Error {\n    code: \"rev_conflict\";\n    rev: number | null;\n}\nexport interface StorageReadCapability {\n    get(key: string): Promise<JsonValue | undefined>;\n    entry(key: string): Promise<StorageValueEntry | undefined>;\n    list(prefix?: string, options?: StorageListOptions): Promise<StorageEntry[]>;\n}\nexport interface StorageWriteCapability {\n    set(key: string, value: JsonValue, options?: StorageWriteOptions): Promise<number>;\n    delete(key: string, options?: StorageWriteOptions): Promise<void>;\n    push(prefix: string, value: JsonValue): Promise<string>;\n    increment(key: string, by?: number): Promise<number>;\n}\nexport type ServerStorageScopeName = \"shared\" | \"readonly\" | \"internal\";\nexport type ServerStorageScope = StorageReadCapability & StorageWriteCapability;\nexport type SqlParameter = string | number | null;\nexport type SqlRow = Record<string, JsonValue>;\nexport interface SqlStatement {\n    query: string;\n    params?: SqlParameter[];\n}\nexport interface SqlResult<Row extends SqlRow = SqlRow> {\n    rows: Row[];\n    rowsRead: number;\n    rowsWritten: number;\n}\nexport interface ServerStorage {\n    shared: ServerStorageScope;\n    readonly: ServerStorageScope;\n    internal: ServerStorageScope;\n    forUser(userId: string): ServerStorageScope;\n    sql<Row extends SqlRow = SqlRow>(query: string, params?: SqlParameter[]): Promise<SqlResult<Row>>;\n    sqlBatch(statements: SqlStatement[]): Promise<SqlResult[]>;\n}\nexport type ServerFileBody = string | ArrayBuffer | Blob | ReadableStream<Uint8Array> | Uint8Array;\nexport interface FileWriteOptions {\n    contentType?: string;\n}\nexport interface FilePullOptions {\n    /** Request headers sent to the source (an Authorization header, say). */\n    headers?: Record<string, string>;\n    /** Stored content type; defaults to the source response's content-type. */\n    contentType?: string;\n}\nexport interface FileGetOptions {\n    /** The request your route is answering. Its Range and conditional headers\n     * (range, if-range, if-none-match, if-match, if-modified-since,\n     * if-unmodified-since) and a HEAD method are forwarded to the platform, so\n     * seeks answer 206 with only the requested bytes, revalidation answers 304,\n     * and HEAD answers headers only. Nothing else (cookies, auth) is copied. */\n    request?: Request;\n    /** An explicit byte range, e.g. \"bytes=0-1023\"; wins over request's Range. */\n    range?: string;\n}\nexport interface FileWriteResult {\n    key: string;\n    url: string | null;\n}\nexport interface PublicFileWriteResult extends FileWriteResult {\n    url: string;\n}\nexport interface PrivateFileWriteResult extends FileWriteResult {\n    url: null;\n}\nexport interface FileMetadata {\n    key: string;\n    size: number;\n    contentType: string;\n    by: AppatoWriter | null;\n    at: number;\n}\nexport interface FileListOptions {\n    cursor?: string;\n    limit?: number;\n}\nexport interface FileListResult {\n    files: FileMetadata[];\n    cursor?: string;\n}\nexport interface FileReadCapability {\n    get(key: string, options?: FileGetOptions): Promise<Response | undefined>;\n    list(prefix?: string, options?: FileListOptions): Promise<FileListResult>;\n}\nexport interface FileWriteCapability<Result extends FileWriteResult = FileWriteResult> {\n    put(key: string, body: ServerFileBody, options?: FileWriteOptions): Promise<Result>;\n    /** Have the platform fetch `url` (https only) and store the response body\n     * under `key`. The bytes never pass through this worker, so there is no\n     * size ceiling beyond the per-file knob. The source must send content-length. */\n    pull(key: string, url: string, options?: FilePullOptions): Promise<Result>;\n    delete(key: string): Promise<void>;\n}\nexport interface FileUrlCapability {\n    url(key: string): string;\n}\nexport type ServerPublicFileScope = FileReadCapability & FileWriteCapability<PublicFileWriteResult> & FileUrlCapability;\nexport type ServerPrivateFileScope = FileReadCapability & FileWriteCapability<PrivateFileWriteResult>;\nexport interface ServerFiles {\n    shared: ServerPublicFileScope;\n    readonly: ServerPublicFileScope;\n    internal: ServerPrivateFileScope;\n    forUser(userId: string): ServerPrivateFileScope;\n}\nexport interface AppatoErrorResponse {\n    error: string;\n}\nexport interface ServerSdk {\n    storage: ServerStorage;\n    files: ServerFiles;\n    email: ServerEmail;\n    handleEmail<Env = unknown>(alias: string, handler: EmailHandler<Env>): void;\n    handleWebhook<Env = unknown>(label: string, handler: WebhookHandler<Env>): void;\n    getUser(request: Request): AppatoUser | null;\n    requireUser(request: Request): AppatoUser;\n    requireMember(request: Request): AppatoUser;\n    getCron(request: Request): CronContext | null;\n    requireCron(request: Request): CronContext;\n    publish(event: string, data: JsonValue, channel?: string): Promise<void>;\n}\nexport interface EmailAddress {\n    address: string;\n    name?: string;\n}\nexport interface IncomingEmailAttachment {\n    id: string;\n    filename: string;\n    contentType: string;\n    size: number;\n    inline: boolean;\n    contentId: string | null;\n    get(): Promise<Response>;\n}\nexport interface IncomingEmail {\n    id: string;\n    deliveryId: string;\n    alias: string;\n    tag?: string;\n    envelopeFrom: string;\n    envelopeRecipients: string[];\n    from: EmailAddress;\n    replyTo: EmailAddress[];\n    to: EmailAddress[];\n    cc: EmailAddress[];\n    subject: string;\n    text: string | null;\n    html: string | null;\n    date: number | null;\n    messageId: string | null;\n    inReplyTo: string | null;\n    references: string[];\n    authentication: {\n        spf: string;\n        dkim: string;\n        dmarc: string;\n        spam: string;\n        virus: string;\n    };\n    attachments: IncomingEmailAttachment[];\n}\nexport type EmailHandler<Env = unknown> = (message: IncomingEmail, env: Env, ctx: AppatoExecutionContext) => void | Promise<void>;\nexport type EmailAttachmentBody = string | ArrayBuffer | Uint8Array | Blob;\nexport interface SendEmailInput {\n    from: {\n        local: string;\n        name?: string;\n    };\n    to: string[];\n    replyTo?: string;\n    subject: string;\n    text?: string;\n    html?: string;\n    category: \"notification\" | \"transactional\";\n    topic?: string;\n    attachments?: Array<{\n        filename: string;\n        contentType: string;\n        data: EmailAttachmentBody;\n    }>;\n}\nexport interface SendEmailResult {\n    batchId: string;\n    queued: Array<{\n        messageId: string;\n        recipient: string;\n    }>;\n    refused: Array<{\n        messageId: string;\n        recipient: string;\n        reason: string;\n    }>;\n}\nexport interface ServerEmail {\n    send(input: SendEmailInput): Promise<SendEmailResult>;\n}\nexport declare function handleEmail<Env = unknown>(alias: string, handler: EmailHandler<Env>): void;\n/**\n * Register the handler for a webhook provisioned under this label. HTTP\n * method, query, headers, and body are passed through unchanged; provider\n * verification and challenge responses belong in the handler.\n */\nexport declare function handleWebhook<Env = unknown>(label: string, handler: WebhookHandler<Env>): void;\nexport declare function getUser(request: Request): AppatoUser | null;\nexport declare function requireUser(request: Request): AppatoUser;\n/** Like requireUser, and additionally refuses a signed-in non-member (403).\n * Use it on routes only workspace members may reach. */\nexport declare function requireMember(request: Request): AppatoUser;\n/** -> { name, scheduledAt, trigger } when the platform scheduler invoked\n * this request, else null. Headers are injected by the platform and can't\n * be spoofed from outside (the dispatch wall strips inbound x-appato-*). */\nexport declare function getCron(request: Request): CronContext | null;\n/** Same, but refuses anything that isn't a real scheduled invocation —\n * use it at the top of every cron handler so a curious member hitting the\n * URL can't run the job by hand. */\nexport declare function requireCron(request: Request): CronContext;\nexport declare const storage: ServerStorage;\n/** Broadcast an ephemeral event to connected browser clients on a channel. */\nexport declare function publish(event: string, data: JsonValue, channel?: string): Promise<void>;\nexport declare const email: ServerEmail;\nexport declare const files: ServerFiles;\n";
...
const MACHINE_LINE_CONTRACT = {
  ...
  APPATO_DEPLOYED: machineLine([
    ...
    machineString("sha"),
    machineString("url"),
    // docs/PUBLIC.md: how many paths this version declared public, and the
    // app's use policy — served anonymously only when policy=public.
    machineInteger("public"),
    machineString("policy"),
  ]),
...
function usage() {
  console.log(`appato ${VERSION}+${BUILD} — build & deploy internal utility apps
...
usage:
  ...
  appato data               what the app has stored: tables, keys per scope,
                            ...
                            (data, files, and logs all take --app <org>/<slug>
                            to name the app when run outside a checkout)
  appato data ls [prefix] [--scope shared|readonly|internal|mine] [--user <id|email>]
                            list a scope's keys (default shared; mine needs
                            --user to say whose)
  appato data get <key> [--scope ...] [--user ...] [--json]
                            print one value as JSON (--json: the value and
                            nothing else, so piping into JSON.parse/jq works)
  ...
  appato types [--check]    write the generated TypeScript declaration for
                            ./_appato.js into the app root. --check fails if
                            it is missing or stale (for local/CI gates)
...
APPS
  ...
  handler. Placement picks the surface:
    <root>/*.ts       server code (Worker modules)
    client/*.ts       browser code — compiles to static assets
                      (client/app.ts -> /client/app.js); load with
                      <script type="module" src="/client/app.js">
    shared/*.ts       BOTH halves — one compile, two surfaces, so a
                      contract both sides must agree on lives in one file.
                      shared/ may import only shared/, and declares no
                      npm:/node: deps (import those in the half that needs
                      them and pass values in)
    everything else   static assets served at their paths — .html, .css,
                      images, fonts, AND .json/.txt data (a .json file is
                      data to fetch(), never an importable module)
  index.html serves / before the fetch handler; prefer static assets for
  the UI. npm packages work in both halves by writing the version into the
  specifier — import { z } from "npm:zod@4" — and push resolves and vendors
  them; always pin. Use the fetch handler for APIs, scheduled jobs, webhooks, and
  genuinely dynamic HTML. The platform handles ALL auth — never build
  login. The /_appato/* URL path is reserved (your handler never sees it).
  Every push deploys.

HOW STORAGE EXECUTES (worth internalizing before designing state)
  Your handler runs in its own isolate, NOT inside the storage engine.
  Every storage call below is ONE independent atomic round trip; between
  two of your calls, other requests run. So get -> modify -> set is a real
  race: use set(key, value, { ifRev }) (compare-and-swap, below) or SQL
  "UPDATE ... WHERE" for contended state. increment is atomic on its own.
  sqlBatch is the ONLY multi-statement transaction; a thrown error rolls
  back nothing already written; writing a value identical to the current
  one still counts as a write (watchers fire).

SERVER SDK — import from "./_appato.js" (injected at deploy; never create it)
  TypeScript project? Run \`appato types\` once to write _appato.d.ts beside
  appato.json, then add \`appato types --check\` to your local/CI gate.
  getUser(request)      -> { id, email, name, org, member } | null  (verified;
                           null for an anonymous visitor on a PUBLIC path)
  requireUser(request)  -> same, or throws a 401 Response
  requireMember(request) -> same, but only a workspace member (401/403)
  ...
  Each scope has the same verbs (keys are relative to the scope):
    ...
    .entry(key)               -> { value, by, at, rev } | undefined
    .set(key, value, { ifRev }?) -> new rev   values: any JSON <= 1MB
    .delete(key, { ifRev }?)
    .list(prefix, { limit, reverse, after }) -> [{ key, value, by, at, rev }]
  ...
  rev is the entry's revision (1 on create, +1 per write). ifRev makes the
  write conditional — a real compare-and-swap, checked atomically: pass the
  rev you read to only-write-if-unchanged, or ifRev: null for only-create-
...
BROWSER SDK — in your served HTML:
  <script type="module">
    ...
    appato.signIn()                   // send a visitor through sign-in (or link
                                      // to /_appato/login); storage/files/
                                      // channel throw until signed in
...
PUBLIC PATHS — internet-facing pages (docs: appato.json "public")
  "public": ["/", "/pricing", "/assets/**", "/api/contact"]   (["/**"] = all)
  Served anonymously ONLY while a workspace admin has set the app's policy
  to Public in the console — push says whether the list is live. Anonymous
  requests carry no cookies, no identity, no appato token (getUser -> null)
  and have no /_appato data plane: public data goes through your own routes
  with the server SDK. Rate-limited per app, 5 MiB body cap, cacheable
  (set your own Cache-Control); your page serves its own Open Graph tags.
  On a custom hostname (appato domain) the same paths serve on that domain;
  www.{apex} redirects to the apex.
...
// How to get a newer CLI, for the copy actually running. A plugin copy
// updates WITH the plugin (auto-update lands it within a session or two);
// `appato upgrade` writes ~/.appato/bin and cannot touch it, so naming it
// here would send the driving agent into a loop of no-op upgrades. The
// nudge deliberately never says "upgrade" for a plugin copy.
const PLUGIN_UPDATE_HINT =
  "this appato comes from the Claude Code plugin, which updates itself within a session or two; " +
  "to update now: /plugin marketplace update appato, then /reload-plugins";

function checkCliVersion(res) {
  const min = res.headers.get("x-appato-cli-min");
  const latest = res.headers.get("x-appato-cli-latest");
  const build = res.headers.get("x-appato-cli-build");
  if (min && semverLt(VERSION, min)) {
    // A hard stop, so the fallback has to be usable right now: the freshly
    // installed ~/.appato/bin copy works by explicit path until the plugin
    // catches up.
    const fix = PLUGIN
      ? `${PLUGIN_UPDATE_HINT} — or run \`appato upgrade\` and use ~/.appato/bin/appato meanwhile`
      : "Run: appato upgrade — then retry the command.";
    throw new Error(`this CLI (v${VERSION}) is too old (server requires v${min}). ${fix}`);
  }
  let behind = null;
  if (latest && semverLt(VERSION, latest)) {
    behind = `appato v${latest} available (you have v${VERSION})`;
  } else if (build && build !== BUILD) {
    // Same VERSION, different bytes: the case a semver comparison cannot see.
    // Commands get ADDED without a contract change, and a month-old plugin
    // cache then reads as current — the agent probes for a command, doesn't
    // find it, and concludes the feature doesn't exist. So the stamp is
    // compared for INEQUALITY, not order: hashes have no order, and "not the
    // build we serve" is the honest thing to say. That does nudge a dev
    // running an unreleased local build; a one-line stderr note is the right
    // price for the agent case.
    behind = `appato ${VERSION}+${build} is deployed (you have ${VERSION}+${BUILD})`;
  }
  if (behind) console.error(`⬆ ${behind} — ${PLUGIN ? PLUGIN_UPDATE_HINT : "run: appato upgrade"}`);
}
...
/**
 * The LIVE version's `public` allowlist (docs/PUBLIC.md U2), read off the
 * governance response — readable by every member, so it never fails where
 * fetchAppState succeeded. Same posture as fetchCrons: the key is replace-all
 * on push and a missing key means "nothing public", so an unreadable list
 * aborts the caller rather than silently unpublishing the app on the next
 * push. A 404 (no such app) reads as none — the caller has already failed
 * or will fail on the app itself.
 */
async function fetchPublicPaths(org, app, verb) {
  const res = await apiFetch(`/api/apps/${org}/${app}/governance`);
  if (res.ok) return /** @type {AppGovernance} */ (await res.json()).publicPaths;
  if (res.status === 404) return [];
  throw new Error(
    `couldn't read ${org}/${app}'s public paths (${res.status}) — stopping instead of ${verb} with an incomplete appato.json, which the next push would treat as "nothing is public". Retry in a moment.`,
  );
}
...
/**
 * Rewrite appato.json's server-owned fields — `title`, `description`,
 * `crons`, `public`, and `email` — in place, preserving every other key (org,
 * app, …). Returns true if anything changed. All five are owned by the
 * platform: title/description may have been edited in the console, and
 * crons/public ride the version and are pushed replace-all — so a stale local
 * value would be stated back over the real one on the next push. `crons` and
 * `public` follow the same rule: the key is deleted when the list is empty.
 */
function writeManifestMeta(root, { title, description, crons, public: publicPaths, email }) {
  ...
  if (publicPaths.length === 0) delete manifest.public;
  else manifest.public = publicPaths;
  if (email !== undefined) {
    manifest.email = {
...
/**
 * Which app an INSPECTION command should look at: `--app org/slug` names one
 * explicitly, so tooling that deliberately lives outside a checkout doesn't
 * have to `cd` into one; with no flag it's the surrounding checkout, exactly
 * as before. Only commands that never touch the checkout or its manifest may
 * use this — `root` is null for an explicit target, so anything reading local
 * files still has to go through appConfig() and say what it needs.
 */
function appTarget(args = []) {
  if (!args.includes("--app")) return appConfig();
  const target = flagValue(args, "--app") ?? "";
  const [org, app, ...extra] = target.split("/");
  if (!org || !app || extra.length > 0) {
    throw new Error(`--app expects org/slug (got ${JSON.stringify(target)})`);
  }
  return { org, app, root: null };
}
...
async function clone(args) {
  ...
  if (version !== null) {
    ...
    // The version's OWN public allowlist (docs/PUBLIC.md U2) — never null:
    // a version that declared nothing has [] and materializes without the key.
    publicPaths = fbody.public;
    ...
    publicPaths = await fetchPublicPaths(org, slug, "cloning");
    ...
    JSON.stringify(
      {
        ...
        ...(crons?.length ? { crons } : {}),
        ...(publicPaths.length ? { public: publicPaths } : {}),
        ...(email
          ? {
              email: {
...
async function show(args = []) {
  ...
  // The version's own public allowlist (docs/PUBLIC.md U2) — served only
  // while the app's policy is Public; empty prints nothing.
  for (const pattern of body.public) console.log(`  public: ${pattern}`);
...
async function push(args = []) {
  const { org, app, title, description, crons, public: publicPaths, root } = appConfig();
  ...
  const meta = {
    ...
    // Stated in full every push, like crons: an absent key is "nothing
    // public" (docs/PUBLIC.md U1) — the fail-closed direction, so there is
    // no "leave it alone" reading to preserve. The server validates it.
    public: Array.isArray(publicPaths) ? publicPaths : [],
  };
  ...
  // Both halves must agree for anything to be public (docs/PUBLIC.md U1):
  // say which half is missing, because a list that isn't served and a policy
  // that exposes nothing are both silent otherwise.
  if (meta.public.length) {
    const live = body.usePolicy === "public";
    console.log(
      `  public: ${meta.public.join(", ")}${
        live
          ? ""
          : ` (not served — this app's policy is "${body.usePolicy}"; a workspace admin sets it to Public in the console)`
      }`,
    );
  } else if (body.usePolicy === "public") {
    console.log('  public: nothing — the policy is Public but appato.json has no "public" list');
  }
  emit("APPATO_DEPLOYED", {
    ...
    sha,
    url: body.url,
    public: meta.public.length,
    policy: body.usePolicy,
  });
}
...
async function sync(args = []) {
  ...
  // schedules, public paths) BEFORE the file comparison and regardless of its outcome.
  ...
  // The LIVE version's public allowlist (docs/PUBLIC.md U2) — the same
  // replace-all rule as crons: a stale local list would be pushed back over
  // the real one, and a missing key would unpublish the app.
  const publicPaths = await fetchPublicPaths(org, app, "syncing");
  if (JSON.stringify(publicPaths) !== JSON.stringify(localPublic ?? [])) metaChanged.push("public");
  const email = await fetchEmailCapability(org, app, "syncing");
  writeManifestMeta(root, {
    title: state.title,
    description: state.description,
    crons,
    public: publicPaths,
...
async function data(args = []) {
  ...
  if (sub === "get") return dataGet(org, app, key, scope, user, args.includes("--json"));
 ...
 *
 * `--json` makes stdout nothing but the value, so `appato data get k --json |
 * jq` works: the trailing APPATO_KV line is what forced every scripted
 * consumer to strip a line before parsing. The value itself is printed the
 * same either way (pretty JSON parses fine); only the machine line is
 * dropped. A miss still tells the human on stderr and exits 1.
 */
async function dataGet(org, app, key, scope, user, json = false) {
  ...
  if (!hit) {
    ...
    if (!json) emit("APPATO_KV", { app: `${org}/${app}`, scope, key, found: false });
    process.exit(1);
  }
...
async function files(args = []) {
  ...
  for (let i = 0; i < args.length; i++) {
    const a = args[i];
    if (a === "--scope" || a === "--user" || a === "--type" || a === "--app" || a === "-o") i++;
    else if (a.startsWith("--")) throw new Error(`unknown flag ${a} — see: appato (usage)`);
    else positionals.push(a);
  }
  const [sub, ...rest] = positionals;
...
const TYPES_FILE = "_appato.d.ts";

/** Write (or verify) the declaration for the injected ./_appato.js module. */
function sdkTypes(args = []) {
  const unknown = args.filter((arg) => arg !== "--check");
  if (unknown.length > 0) throw new Error("usage: appato types [--check]");
  ...
  if (args.includes("--check")) {
    if (current !== SDK_TYPES) {
      throw new Error(`${TYPES_FILE} is missing or stale — run: appato types`);
    }
    console.log(`✓ ${TYPES_FILE} is up to date`);
    return;
  }

  if (current === SDK_TYPES) {
    console.log(`Already up to date: ${TYPES_FILE}`);
    return;
  }
  writeFileSync(path, SDK_TYPES);
  console.log(`✓ wrote ${TYPES_FILE} (local types; never deployed)`);
}
...
/**
 * What a shell would actually run for `appato`, relative to the wrapper we
 * just wrote in `binDir`. Walking PATH in order is the point: the observed
 * failure is not a missing directory but an OLD `appato` EARLIER on PATH,
 * which keeps answering `--version` with a build the server refuses.
 *
 * Pure (the executability check is injected) so it's testable without a
 * filesystem. Returns `onPath: false` when binDir isn't on PATH at all, and
 * otherwise `shadowedBy` = the winning path when it isn't ours.
 */
function pathShadow(pathString, binDir, isExecutable) {
  // An EMPTY PATH component means the current directory — a shell would run
  // ./appato from it — so it maps to "." rather than being dropped.
  const dirs = (pathString ?? "").split(delimiter).map((dir) => dir || ".");
  if (!dirs.includes(binDir)) return { onPath: false, shadowedBy: null };
  for (const dir of dirs) {
    if (dir === binDir) break;
    const candidate = join(dir, "appato");
    if (isExecutable(candidate)) return { onPath: true, shadowedBy: candidate };
  }
  return { onPath: true, shadowedBy: null };
}
...
async function install() {
  ...
  const { onPath, shadowedBy } = pathShadow(process.env.PATH, binDir, executable);
  if (!onPath) {
    console.log(
      `Not on PATH — use ${wrapper} directly, or add: export PATH="$HOME/.appato/bin:$PATH"`,
    );
  } else if (shadowedBy && PLUGIN) {
    // Expected in Claude Code: the plugin's bin/ is ahead on PATH by design
    // and updates with the plugin, so "remove it" would be the wrong advice.
    console.log(
      `Note: bare \`appato\` here comes from the Claude Code plugin, which updates itself — this copy at ${wrapper} is for your terminal.`,
    );
  } else if (shadowedBy) {
    console.log(
      `⚠ ${shadowedBy} comes first on PATH and shadows ${wrapper} — \`appato\` still runs the old build. Remove it or put $HOME/.appato/bin first on PATH.`,
    );
  }
...
try {
  switch (command) {
    ...
    case "--version":
    case "version":
      // VERSION alone can't tell two builds apart — the stamp can.

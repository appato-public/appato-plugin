...
2. **Create**: derive a short kebab-case slug from the app's purpose (e.g.
   "PTO tracker" → `pto-tracker`). In a new directory for the app, run:
   ...
   This writes the app manifest, `appato.json`:
...
3. **Write code** (conventions below), then push and share the printed URL
   with the user.
4. **Push frequently, always with a change summary.** Run
...
5. **If push reports a deploy error**, read the error, fix the code, and
   push again. The previous working version keeps serving until a push
   succeeds.
6. **Before ending ANY response where you touched app files, push — and
...
7. **If any appato output mentions an upgrade** (or says the CLI is too
   old), run `appato upgrade`, then retry.

## CLI output contract

The last line of `appato push` and `appato status` is machine-readable —
space-separated `key=value` pairs (values JSON-quoted if they contain
spaces). Parse it for your deploy-status statement; don't scrape the
human-readable lines above it.
...
- `APPATO_STATUS app=<org>/<slug> deployed_version=<n|none>
  deployed_at=<ms-epoch|never> dirty=<true|false> sha=<12-hex> url=<url>` —
  `dirty=true` means local files differ from the pushed version.
